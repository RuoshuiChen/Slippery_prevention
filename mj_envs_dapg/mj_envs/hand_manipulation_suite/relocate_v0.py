...
class RelocateEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        self.do_simulation(a, self.frame_skip)  # implement this actions
    ...
    def reset_model4Koopman(self, ori, init_pos, init_vel):
        qp = self.init_qpos.copy()
        # qp[:30] = np.random.random(30) * init_pos  # randomize the initial pose of the hands
        qp[:30] = np.random.uniform(-1, 1, 30) * init_pos
        # qp[30:] = np.random.random(6) * init_pos * 0.2
        qv = self.init_qvel.copy()
        # qv[:30] = np.random.random(30) * init_vel
        qv[:30] = np.random.uniform(-1, 1, 30) * init_vel
        # qv[30:] = np.random.random(6) * init_vel * 0.2
        self.set_state(qp, qv)
