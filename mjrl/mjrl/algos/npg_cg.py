...
class NPG(BatchREINFORCE):
    def __init__(self, env, policy, baseline,
        ...
        The problem of NPG is that FIM (H) and its inverse are very expensive to compute. The subsequent algoritm TRPO improves this by using an approximation
    ...
    def HVP(self, observations, actions, vector, regu_coef=None):
        # to compute the matrix-vector product between the FIM and any arbitrary vecots (input vector, in this case, it is vpg or dapg)
        # vector -> dapg_grad (or vpg)
        # regu_coef -> 0.0001 in default
        ...
        if self.hvp_subsample is not None and self.hvp_subsample < 0.99:
            ...
            obs = observations  
            act = actions    
        old_dist_info = self.policy.old_dist_info(obs, act) # return: [LL, mean, self.old_log_std] 
        new_dist_info = self.policy.new_dist_info(obs, act)
        mean_kl = self.policy.mean_kl(new_dist_info, old_dist_info)  # compute kl_divergence (a scalar)
        # this one is for the kl divergence between the distributions corrsponding to the two mean parameters.
        grad_fo = torch.autograd.grad(mean_kl, self.policy.trainable_params, create_graph=True)
        flat_grad = torch.cat([g.contiguous().view(-1) for g in grad_fo])  # flat_grad has the same dimension as vec (both all the gradient wrt to training param)
        h = torch.sum(flat_grad*vec) # a scalar  
        ...
        return hvp_flat + regu_coef*vector  # second term, damping term, which is the original policy gradient (vpg)
        # the natural gradient is added with the vanilla gradient

    def build_Hvp_eval(self, inputs, regu_coef=None):
        def eval(v):  #when calling the returned function eval, inputs will always be [observations, actions]
            full_inp = inputs + [v] + [regu_coef]
            Hvp = self.HVP(*full_inp)
            return Hvp
        return eval # return a function
    ...
    def train_from_paths(self, paths):
        ...
        npg_grad = cg_solve(hvp, vpg_grad, x_0=vpg_grad.copy(),
                            cg_iters=self.FIM_invert_args['iters'])  # correponds to F_{\theta_k}^{-1}g in the paper
