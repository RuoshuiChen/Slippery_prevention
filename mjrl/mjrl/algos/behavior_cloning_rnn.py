...
class BC:
    ...
    def mse_loss(self, data, idx=None):
        ...
        if type(data['observations']) is torch.Tensor:
            idx = torch.LongTensor(idx)
        obs = data['observations'][idx]
        act_expert = data['expert_actions'][idx]  # from demonstration
        ...
        act_pi = self.policy.model(obs)  # obtain the RNN model's actions (hidden states at each time step)
    ...
    def fit(self, data, suppress_fit_tqdm=False, **kwargs):
        ...
        ts = timer.time()
        num_samples = data["observations"].shape[0]  # number of demo data
        # print(num_samples, self.mb_size)
        # input()
        ...
        if (kwargs['save_policy']):
            pickle.dump(self.policy, open('/home/abhineet/STAR-Lab/hand_manipulation/hand_dapg/dapg/examples/logs/ncp_rl/' + kwargs['task'] + '-v0-bc.pickle', 'wb'))
    ...
    def train(self, **kwargs):
        observations = np.array([path["observations"] for path in self.expert_paths])
        expert_actions = np.array([path["actions"] for path in self.expert_paths])
