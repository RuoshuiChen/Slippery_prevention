...
class BC:
    ...
    def compute_transformations(self):
        ...
        if self.expert_paths == [] or self.expert_paths is None:
            ...
            observations = np.concatenate([path["observations"] for path in self.expert_paths])  # path is a dict
    ...
    def fit(self, data, suppress_fit_tqdm=False, **kwargs):
        ...
        if (kwargs['save_policy']):
            pickle.dump(self.policy, open('/home/abhineet/hand_manipulation/hand_dapg/dapg/policy_demos/bc_policies/' + kwargs['task'] + '-v0-bc.pickle', 'wb'))
    ...
    def train(self, **kwargs):
        # what will happen if they are empty?
