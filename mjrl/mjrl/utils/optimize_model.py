...
def fit_data(model, x, y, optimizer, loss_func, batch_size, epochs):
    ...
    for ep in range(epochs):
        ...
        for mb in range(num_steps):
            ...
            batch_y = y[data_idx]
            optimizer.zero_grad()
            yhat = model(batch_x) # model output using the current baseline model
            loss = loss_func(yhat, batch_y)  # not the TR method, but a simple MLP training with labelled data to minimize the L2 norm error
            loss.backward()
            optimizer.step()
            ep_loss += loss.detach()
