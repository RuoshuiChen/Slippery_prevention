...
def do_rollout(
    ...
    horizon = min(horizon, env.horizon)  # env.horizon is muych smaller than default horizon (1e6)
    paths = []
    for ep in range(num_traj):
        ...
        agent_infos = []
        env_infos = []
        o = env.reset() # original observation data, initialization
        done = False
        t = 0

        if isinstance(policy.model, NCPNetwork):  # natural circuit policy
            hidden_state = np.zeros((1, policy.model.rnn_cell.state_size))
        if isinstance(policy.model, RNNNetwork): # RNN
            hidden_state = (np.zeros((1, 1, policy.model.rnn_cell.hidden_size)), np.zeros((1, 1, policy.model.rnn_cell.hidden_size)))
        while t < horizon and done != True:  # generate a single traj
            if isinstance(policy.model, NCPNetwork):
                a, agent_info = policy.get_action(o, hidden_state) # we explicitly provide with the hideen state at beginning
                # the hidden state will also be passed to the next time step
                hidden_state = agent_info['hidden_state']
            elif isinstance(policy.model, RNNNetwork):
                a, agent_info = policy.get_action(o, hidden_state) # we explicitly provide with the hideen state 
                # the hidden state will also be passed to the next time step
                hidden_state = agent_info['hidden_state']
            else:
                a, agent_info = policy.get_action(o) # FC network when using gaussian_mlp as policy
                # a.shape = 30 -> for the relocation task
                # return [action, {'mean': mean, 'log_std': self.log_std_val, 'evaluation': mean}]
                # a -> action = mean + noise
                # agent_info -> {'mean': mean, 'log_std': self.log_std_val, 'evaluation': mean}
            if eval_mode: # eval_mode -> no noise is added to mean
                a = agent_info['evaluation']
            env_info_base = env.get_env_infos()  # seems always to be {}
            next_o, r, done, env_info_step = env.step(a) # apply the action to the environment and obtained the observation at next step and the reward value after the execution
